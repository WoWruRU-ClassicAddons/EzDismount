...
function EzDismount_onload()
	...
	EzDPlayer = (UnitName("player")..L[" of "]..GetCVar("realmName"))
	EZDismount_DetPlayer:SetText(EzDPlayer..L[" Server"]);
	...
	if (EzDismount_Config[EzDPlayer] == nil) then
  	...
  	DEFAULT_CHAT_FRAME:AddMessage(EzDismount_fullver .. L[" Loaded ##  Use /ezd or /ezd help"], 0.0, 1.0, 0.0);
...
function EzDismount_Refresh()
	...
	-- Mounts
	EzDismount_Text_Status:SetText(L["Automatic dismounting is :"]);
	if ( EzDismount_Config[EzDPlayer]["Dismount"] == "OFF" ) then
	...
	-- Auctioneer Dismount
	EzDismount_Text_Auction:SetText(L["Automatic auctioneer dismount is :"]);
	if ( EzDismount_Config[EzDPlayer]["Auction"] == "OFF" ) then
	...
	-- Auto-Stand
	EzDismount_Text_Stand:SetText(L["Automatic stand from sit is :"]);
	if ( EzDismount_Config[EzDPlayer]["Stand"] == "OFF" ) then
	...
	-- Shaman
	EzDismount_Text_Shaman:SetText(L["Auto-cancel of Ghostwolf is :"]);
	...
	-- Druid
	EzDismount_Text_Druid:SetText(L["Auto-cancel of shapeshifts is :"]);
	...
	-- Moonkin
	EzDismount_Text_Moonkin:SetText(L["Auto-cancel of Moonkin form :"]);
	...
	-- Shadowform
	EzDismount_Text_Shadowform:SetText(L["Auto-cancel of Shadowform :"]);
